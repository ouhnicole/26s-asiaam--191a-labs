...
cafes.forEach(cafe => {
    new maplibregl.Marker()
        ...
        .setPopup(new maplibregl.Popup({ offset: 25 })
            .setHTML(`<strong>${cafe.name}</strong><br>${cafe.description}<br><br><em>Café rating: ${cafe.rating} ★</em>`))
        .addTo(map);
});

function focusCafe(name) {
    ...
    new maplibregl.Popup({ offset: 25 })
        .setLngLat(cafe.coords)
        .setHTML(`<strong>${cafe.name}</strong><br>${cafe.description}<br><br><em>Rating: ${cafe.rating} ★</em>`)
        .addTo(map);
}

document.querySelectorAll('.cafe-button').forEach(button => {
    button.addEventListener('click', () => focusCafe(button.dataset.cafe));
});
